...
t_corn_param	calc_ints_param_of_corn(t_corn *c, t_ray ray)
{
	...
	param.vec_cross_origin2pe_n = cross(param.vec_origin2pe, c->axis);
	...
	param.dot_origin2pe_n = dot(param.vec_origin2pe, c->axis);
	param.vec_norm_de_x_n = norm(param.vec_cross_de_n);
	param.vec_norm_origin2pe_x_n = norm(param.vec_cross_origin2pe_n);
	return (param);
}
...
t_d_param	calc_d_param_of_corn(t_corn_param p)
{
	...
	d_param.b = 2.0 * dot(p.vec_cross_de_n, p.vec_cross_origin2pe_n) \
 				- 2.0 * pow(p.ratio_r_h, 2) * p.dot_de_n * p.dot_de_n;
	d_param.c = pow(p.vec_norm_origin2pe_x_n, 2) \
