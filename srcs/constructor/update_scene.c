...
{
	scene->ambient_color = color_k1c1(scene->brightness / 255.0, scene->ambient_color);
}
...
void	update_lights(t_scene_info *scene)
{
	...
	{
		light = node->content;
		light->light_color = color_k1c1(light->brightness / 255.0, light->light_color);
		node = node->next;
	}
}
...
void	get_radius(t_obj *obj)
{
	...
	if (obj->type == BALL)
		...
		obj->shape_data->cylinder->radius = obj->shape_data->cylinder->diameter / 2.0;
	else
...
void	update_objs(t_scene_info *scene)
{
	...
	while (node)
	{
		...
		if (obj->obj_color->is_checker)
			obj->obj_color->checker_color = color_k1c1(1.0 / 255.0, obj->obj_color->checker_color);
		get_radius(obj);
		node = node->next;
	}
}
